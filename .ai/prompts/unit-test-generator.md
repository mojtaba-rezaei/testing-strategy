...
| Missing mapping example | Generate generic test data | "Do you have example input/output data for `OrderMapper`? Even a bad example (data that should fail) helps me generate better tests. You can also choose to proceed without examples." |

### ALWAYS Ask for Mapping Examples

**This is a BLOCKING rule.** For every mapper/converter class, the AI agent MUST ask the user whether they have example input/output data — even before generating tests. Real-world examples (good or bad) dramatically improve test accuracy.

**When to ask:** Before generating tests for ANY mapper or converter class, regardless of whether CSV mapping specs are available.

**MANDATORY prompt for each mapping:**

> "I found the mapper class `{ClassName}`. Do you have example input/output data for this mapping?
>
> - **A good example** (known correct input → expected output) lets me generate precise assertions
> - **A bad example** (input that should fail or produce errors) lets me generate negative test cases
> - **No example** — I can still generate tests based on code analysis and CSV specs (if available), but the assertions will be less precise
>
> You can provide:
> - A JSON/XML file with sample input and expected output
> - Paste the example directly in chat
> - Or choose to **proceed without examples**"

**How examples improve tests:**

| Scenario | Without Example | With Example |
|----------|-----------------|--------------|
| Happy path test | Generic assertion: `result.Should().NotBeNull()` | Precise assertion: `result.OrderId.Should().Be("ORD-12345")` |
| Edge case test | Guessed boundary values | Real-world edge case from production data |
| Error handling test | Generic `ThrowsException` | Specific exception with known bad input |

**Rules:**
1. Ask for examples **per mapper class**, not once for the whole project
2. If the user provides a good example, use it as the primary test data in `SampleData.cs`
...
**This is a MANDATORY rule.** After test generation and verification are complete, the AI agent MUST clean up all generated test artifacts from the repository. Test results, coverage reports, and build outputs should never be committed to source control.

**What to clean up:**
- `TestResults/` folders (generated by `dotnet test`)
- `coverage.cobertura.xml` and other coverage report files
- `*.trx` test result files
- `coveragereport/` folders (generated by coverage report tools)
- `bin/` and `obj/` folders inside the generated test project
- Any `Summary.txt` or temporary build output files

**When to clean up:** After Step 8 (Build & Test Verification) passes successfully, and before marking the task as complete.

**Cleanup command:**
```bash
# Remove test results and coverage artifacts
Remove-Item -Recurse -Force TestResults
Remove-Item -Recurse -Force coveragereport
Get-ChildItem -Recurse -Filter "*.trx" | Remove-Item -Force
Get-ChildItem -Recurse -Filter "coverage.cobertura.xml" | Remove-Item -Force

# Remove build outputs from test project
Remove-Item -Recurse -Force tests/unit/**/bin
Remove-Item -Recurse -Force tests/unit/**/obj
```

**Rules:**
1. ALWAYS clean up after successful test verification
2. If cleanup fails, warn the user but do not block task completion
3. Verify that `.gitignore` includes patterns for test artifacts (`TestResults/`, `coveragereport/`, `*.trx`)
4. Never leave test result files in the working directory
...
**MANDATORY follow-up — Ask for examples per mapper (see Core Principles):**

After resolving CSV specs (found or not), ALWAYS ask the user for example input/output data for **each** mapper class. This is a separate, additional prompt — do not skip it even when CSV specs are available. See the [ALWAYS Ask for Mapping Examples](#always-ask-for-mapping-examples) core principle for the exact prompt template and rules.
...
**Step 1b: Ask for Mapping Examples (BLOCKING for Mappers)**
```
Actions:
  1. For EACH mapper/converter class discovered in Step 0:
     → Ask the user if they have example input/output data (good or bad examples)
     → Offer the choice to proceed without examples
  2. If the user provides a good example:
     → Use it as primary test data in SampleData.cs
     → Generate precise field-level assertions based on the example
  3. If the user provides a bad example:
     → Generate a dedicated error/validation test for it
  4. If the user chooses no examples:
     → Continue with code analysis and CSV specs only
     → Add TODO comments marking where real examples would improve assertions
  5. NEVER skip this step — even when CSV specs are available
```
...
**Step 9: Clean Up Test Artifacts (MANDATORY)**
```
After Step 8 passes successfully:
  1. Remove TestResults/ folders generated by dotnet test
  2. Remove coverage.cobertura.xml and other coverage report files
  3. Remove *.trx test result files
  4. Remove coveragereport/ folders
  5. Remove bin/ and obj/ folders from the generated test project
  6. Remove any Summary.txt or temporary build output files
  7. Verify .gitignore includes patterns for test artifacts
  8. If cleanup fails, warn the user but do not block completion
```
...
Before finalizing:
  ...
  ✓ Repo is clean — no generated test results left behind
```
...
MAPPING EXAMPLES: {examplePaths | "NONE — user chose to proceed without" | "NOT YET ASKED — must ask per mapper"}
...
9. For mappers: ask for example input/output data per mapper (good or bad)
10. Fill in existing stubbed/TODO tests with real logic
11. ASK about any unclear behavior — do NOT guess
12. Verify all tests compile and pass before completing
13. Clean up all test artifacts (TestResults/, coverage reports, *.trx) before finishing

WORKFLOW:
- Step 0: Discover all testable components (use Explore subagent)
- Step 1: Check for CSV mapping specs (BLOCKING for mappers)
- Step 1b: Ask for mapping examples per mapper class (BLOCKING)
- Step 2-8: Follow Generation Workflow in Section 8.1
...
- [ ] User was prompted for example input/output data per mapper class
- [ ] User-provided good examples used as primary test data in SampleData.cs
- [ ] User-provided bad examples used to generate error/validation tests
- [ ] If user chose no examples, TODO comments added where examples would improve assertions
...
### 9.9 Cleanup
- [ ] `TestResults/` folders removed
- [ ] `coverage.cobertura.xml` and coverage report files removed
- [ ] `*.trx` test result files removed
- [ ] `coveragereport/` folders removed
- [ ] `bin/` and `obj/` folders removed from test project
- [ ] `.gitignore` verified to include test artifact patterns
- [ ] No generated test results remain in the working directory

### 9.10 Information Integrity
...
✅ Always ask for mapping examples (good or bad) per mapper class  
...
✅ Clean up all test artifacts — leave the repo clean  
